...
def adding_players_roles(player, position, skill):
    player_info.setdefault(player, {})
    player_info[player].setdefault(position, 0)
    player_info[player][position] = max(player_info[player][position], int(skill))


commands = {
    'vs': duel_players,
    '->': adding_players_roles
}

while command_input != "Season end":
    command = " vs " if " vs " in command_input else " -> "
    commands[command.strip()](*command_input.split(command))
    command_input = input()
